...
class Google(Scraper):
    ...
    def _get_initial_cursor_and_jobs(self) -> Tuple[str, list[JobPost]]:
        ...
        # Use requests directly with full browser headers
        ...
        log.info(f"Google response status: {response.status_code}, length: {len(response.text)}")

        # Debug: check if jobs data exists
        if "520084652" in response.text:
            log.info("Jobs data key found in response")
        else:
            log.warning("Jobs data key NOT found — Google may be blocking or format changed")
            # Save response for inspection
            with open("google_debug_response.html", "w", encoding="utf-8") as f:
                f.write(response.text)
            log.info("Saved raw response to google_debug_response.html")
