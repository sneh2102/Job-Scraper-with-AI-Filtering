...
def print_evaluation_results(
    ...
    if hasattr(evaluation, "scores") and evaluation.scores:
        for category_name, category_data in evaluation.scores.model_dump().items():
            if category_data is None:  # skip removed/optional fields
                continue
    ...
    if hasattr(evaluation, "scores") and evaluation.scores:
        ...
        for cat_id, icon in category_icons.items():
            cat_score = getattr(evaluation.scores, cat_id, None)
            if cat_score:
                capped = min(cat_score.score, category_maxes[cat_id])
                print(f"{icon} {capped}/{cat_score.max}")
