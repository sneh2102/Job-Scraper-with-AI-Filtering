...
def transform_evaluation_response(
    ...
    if evaluation and hasattr(evaluation, "scores"):
        ...
        csv_row["open_source_score"] = scores.open_source.score if scores.open_source else "N/A"
        csv_row["open_source_max"] = scores.open_source.max if scores.open_source else "N/A"
        ...
        total_score = (
            (scores.open_source.score if scores.open_source else 0)
            + scores.self_projects.score
            + scores.production.score
            + scores.technical_skills.score
        )
        total_max = (
            (scores.open_source.max if scores.open_source else 0)
